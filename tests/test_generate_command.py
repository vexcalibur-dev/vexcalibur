...
from vexcalibur.domain import VulnerabilitySourceInputError
...
def test_github_command_translates_source_preflight_input_errors(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    source_error = VulnerabilitySourceInputError("source preflight rejected the input")
    ...
    with pytest.raises(SbomError, match="source preflight rejected the input") as raised:
        request.execute()
