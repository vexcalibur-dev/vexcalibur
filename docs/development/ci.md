...
- release-evidence fixture generation from that same local wheel. CI generates
  the zero-finding production bundle twice and requires byte equality, then
  generates the synthetic `in_triage` all-format bundle twice and applies the
  same determinism requirement. It runs the pinned OpenVEX and CSAF validators
  and checks cross-format assertions.
...
Reproduce the self-release evidence job with a freshly built local wheel:
...
The command uses no credentials and does not publish the generated temp
bundles. See [Build and review self-release
evidence](../how-to/build-release-evidence.md) for exact wheel selection,
expected output, failure handling, and the inspectable local bundle.
